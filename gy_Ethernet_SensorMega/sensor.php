  ...
  if (isset($_GET['value']))
  {
    $deger =$_GET['value'];
     ...
     $subject2 = sqlsrv_fetch_array($subject_set,SQLSRV_FETCH_ASSOC);
      ...
      {
         $query = "insert into DP_Kart (ID,IP,TIPI,SIRA,DEGER,chg_date) values ('{$KartID}','{$ip}',1,0,{$deger},'{$Tarih}')";
	  }
...
$query = "insert into DP_Log (ID,IP,TIPI,SIRA,DEGER,chg_date) values ('{$KartID}','{$ip}',1,0,{$deger},'{$Tarih}')";
$stmt = sqlsrv_prepare($baglanti,$query,array());
        if( sqlsrv_execute( $stmt ) === false ) 
            echo "hata:",sqlsrv_errors();
